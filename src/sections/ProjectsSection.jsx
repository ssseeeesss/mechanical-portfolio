...
const RESTRICTED_MODEL_PROJECTS = new Set(['umi-h', 'g1-exoskeleton-glove']);

function canShowModel(project) {
  return Boolean(project.model?.src) && !RESTRICTED_MODEL_PROJECTS.has(project.id);
}
...
export default function ProjectsSection() {
  ...
  const warmModel = useCallback((project) => {
    if (!canShowModel(project) || !allowsModelPreload()) return;
  ...
  useEffect(() => {
    ...
    const warmFirstModel = () => {
      if (!cancelled) {
        const firstModelProject = projects.find(canShowModel);
        if (firstModelProject) warmModel(firstModelProject);
      }
    };
  ...
  const openModel = useCallback((project) => {
    if (!canShowModel(project)) return;
    warmModel(project);
